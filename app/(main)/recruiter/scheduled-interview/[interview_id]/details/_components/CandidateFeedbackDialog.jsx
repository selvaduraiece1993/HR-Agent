...
function CandidateFeedbackDialog({ candidate }) {
  ...
  const fetchCandidateCV = useCallback(async () => {
...
We're pleased to inform you that based on your recent interview performance, you've been selected to move forward in our hiring process for the ${candidate?.job_position || 'the position'} role.
...
${candidate?.email || 'No Email'}`,
    ...
    rejected: `Subject: Update on Your Application for ${candidate?.job_position || 'the position'}

Dear ${candidate?.fullname || 'Candidate'},

Thank you for taking the time to interview with us for the ${candidate?.job_position || 'the position'} position. We appreciate the effort you put into the process.
...
${candidate?.email || 'No Email'}`,
    ...
    reevaluate: `Subject: Request for Additional Evaluation for ${candidate?.job_position || 'the position'}

Dear ${candidate?.fullname || 'Candidate'},

Thank you for your recent interview for the ${candidate?.job_position || 'the position'} role. While we found several strengths in your application, we'd like to gather some additional information before making a final decision.
...
${candidate?.email || 'No Email'}`,
  ...
  const handleEmailAction = (templateType) => {
    ...
    const subject = emailTemplates[templateType]
      .split('\n')[0]
      .replace('Subject: ', '');
