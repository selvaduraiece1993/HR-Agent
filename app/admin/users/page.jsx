...
function UserManagement() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const interviewTableName=process.env.INTERVIEWS_TABLE_NAME;
      const interviewResultsTableName=process.env.INTERVIEW_RESULTS_TABLE_NAME; 
      const usersWithStats = await Promise.all(
        data.map(async (user) => {
          const { count: interviewCount } = await supabase
            .from(interviewTableName)
            .select('*', { count: 'exact', head: true })
            .eq('userEmail', user.email);

          const { count: candidateCount } = await supabase
            .from(interviewResultsTableName)
            .select('*', { count: 'exact', head: true })
  ...
  const getStatusColor = (user) => {
    if (user.banned) return 'text-orange-600 bg-teal-50';
    if (user.interviewCount > 0) return 'text-teal-600 bg-teal-50';
    if (user.created_at) return 'text-teal-600 bg-teal-50';
  ...
  const deleteUser = async (userId) => {
    try {
      ...
      const { error: resultsError } = await supabase
        .from(DB_TABLES.INTERVIEWS_RESULTS)
        .delete()
  ...
  const handleAddAdmin = async (e) => {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from(DB_TABLES.USERS).insert([
        {
          email: adminEmail,
          name: adminName,
          role: 'admin',
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          {user?.email?.includes('@superadmin') && (
            <Button onClick={() => setShowAddAdmin(true)} className="bg-teal-600 text-white">
              + Add Admin
            </Button>
          )}
        ...
        <Dialog open={showAddAdmin} onOpenChange={setShowAddAdmin}>
          <DialogContent>
            ...
            <form onSubmit={handleAddAdmin} className="space-y-4">
              ...
              <DialogFooter>
                <Button type="submit" disabled={addingAdmin} className="bg-teal-600 text-white w-full">
          ...
          <div className="flex gap-4">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                className={showBanned ? "bg-orange-600 hover:bg-orange-700" : ""}
              >
                   ...
                   <div className="flex items-center space-x-6">
                     ...
                     <div className="flex items-center space-x-2">
                       ...
                       <Button
                         ...
                         className="text-teal-600 hover:text-amber-700"
                       >
                         Debug
                       </Button>
                       ...
                       <AlertDialog>
                         <AlertDialogTrigger asChild>
                           <Button
                             variant="outline"
                             size="sm"
                             className={user.banned ? "text-teal-600 hover:text-teal-700" : "text-orange-600 hover:text-orange-700"}
                           >
                             {user.banned ? (
                               <>
                         ...
                         <AlertDialogContent>
                           ...
                           <AlertDialogFooter>
                             ...
                             <AlertDialogAction
                               ...
                               className={user.banned ? "bg-teal-600 hover:bg-teal-700" : "bg-orange-600 hover:bg-orange-700"}
                             >
                               {user.banned ? 'Unban User' : 'Ban User'}
                             </AlertDialogAction>
                           </AlertDialogFooter>
                       ...
                       <AlertDialog>
                         <AlertDialogTrigger asChild>
                           <Button
                             variant="outline"
                             size="sm"
                             className="text-orange-600 hover:text-orange-700 hover:bg-teal-50"
                           >
